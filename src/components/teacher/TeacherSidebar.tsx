...
export default function TeacherSidebar({ profile }: { profile?: Profile | null }) {
    ...
    <aside className="w-full md:w-64 bg-campus-950 text-campus-100 flex flex-col shrink-0">
      ...
      <div className="p-6 border-b border-campus-900 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-campus-800 flex items-center justify-center text-white shadow-xs shrink-0">
            <Sparkles className="w-5 h-5 text-campus-300" />
          </div>
          <div className="min-w-0">
            <div className="font-extrabold text-sm text-campus-100 truncate">Teacher Hub</div>
            <div className="text-[10px] text-campus-300 font-bold uppercase truncate">EBAUB Faculty</div>
          </div>
        </Link>
      </div>
      ...
      <nav className="p-4 space-y-1.5 flex-1 overflow-y-auto text-xs font-semibold">
        {navItems.map(item => {
          ...
          return (
            ...
            >
              <Icon className={`w-4 h-4 shrink-0 ${isActive ? 'text-campus-400' : 'text-campus-400'}`} />
