...
export default function Footer({ settings = DEFAULT_SITE_SETTINGS }: Props) {
  const general = settings?.general || DEFAULT_SITE_SETTINGS.general;
  const contact = settings?.contact || DEFAULT_SITE_SETTINGS.contact;
  const socials = settings?.socials || DEFAULT_SITE_SETTINGS.socials;
    ...
    <footer className="bg-emerald-950 text-slate-100 text-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3">
              ...
              <span className="font-extrabold text-base sm:text-lg text-white leading-tight">
                {general.site_name || 'EXIM Bank Agricultural University Bangladesh'}
              </span>
            </div>
            ...
            <div className="space-y-3 text-slate-300 text-xs sm:text-sm">
              <div className="flex items-start gap-3">
                ...
                <span className="whitespace-pre-line leading-relaxed">{contact.campus_address}</span>
              </div>
              <div className="flex items-center gap-3">
                <Mail className="w-5 h-5 text-emerald-400 shrink-0" />
                <a href={`mailto:${contact.inquiries_email}`} className="hover:text-amber-400 transition-colors">
                  {contact.inquiries_email}
                </a>
              </div>
              <div className="flex items-center gap-3">
                <Phone className="w-5 h-5 text-emerald-400 shrink-0" />
                <a href={`tel:${contact.hotline_phone}`} className="hover:text-amber-400 transition-colors">
                  {contact.hotline_phone} {contact.admissions_phone ? `(Admissions: ${contact.admissions_phone})` : ''}
                </a>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3 font-medium text-slate-300">
              <li>
                <Link href={socials.student_portal_url || "/student"} className="hover:text-amber-400 transition-colors duration-300 flex items-center gap-2">
                  <span>Student Dashboard</span>
                  <ExternalLink className="w-3.5 h-3.5 opacity-70" />
                </Link>
              </li>
              <li>
                <Link href={socials.teacher_portal_url || "/teacher"} className="hover:text-amber-400 transition-colors duration-300 flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-slate-300 mb-6 text-xs sm:text-sm">
              {general.tagline || 'Stay updated with the latest news, events, and academic announcements.'}
            </p>
            <div className="flex items-center gap-4">
              ...
              {!socials.facebook_url && !socials.linkedin_url && !socials.youtube_url && (
                <a 
      ...
      <div className="border-t border-emerald-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left text-slate-400 text-xs sm:text-sm">
          <p>
            &copy; {new Date().getFullYear()} {general.short_name || 'EBAUB'}. All rights reserved.
          </p>
