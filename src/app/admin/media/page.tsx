    ...
    <div className="space-y-8 max-w-5xl mx-auto">
      <div>
        <h1 className="text-2xl font-extrabold text-white flex items-center gap-2">
          <ImageIcon className="w-6 h-6 text-emerald-400" /> Media Library Manager
        </h1>
        <p className="text-xs text-slate-400 mt-1">Upload and manage image assets for news, notices, and banners</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {mediaItems.map(item => (
          <div key={item.id} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden space-y-3 p-3">
            ...
            <div className="text-xs text-slate-300 font-bold truncate">{item.filename}</div>
